...
script_dir = os.path.dirname(os.path.abspath(__file__))
data_file = root_dir + \
    'preprocessed_inputs/paper1/fingerprint_history_window_3_4_100_median.csv'
scaler_file = script_dir+'/files/model_3_scaler_autokeras.pkl'
model_file = script_dir+'/files/model_3_autokeras.tf'
random_seed = 42

# Autokeras config
max_trials = 20
autokeras_project_name = 'posicionamiento_modelo3'
autokeras_folder = root_dir+'/tmp/autokeras_training/'
...
Xmap = Xmap.to_numpy().astype(np.int32)

# Construimos el modelo

# Entradas
inputSensors = ak.Input(shape=X.shape[1])
InputMap = ak.Input(shape=Xmap.shape[1])
