...
fingerprint_history_train_file = script_dir+'/dataset_processed_csv/fingerprint_history_train_v3.csv'                                  #Salida del csv de entrenamiento
fingerprint_history_test_file = script_dir+'/dataset_processed_csv/fingerprint_history_test_v3.csv'                                    #Salida del csv de tests
...
def close_time_group(data_segment, final_list, sensors_mac):
    ...
    for mac in sensors_mac:
        ...
        if(len(macrssi) > 0):
            subdata['rssi_'+mac] = round(macrssi['rssi'].mean())
            subdata['state_'+mac] = 1
        else:
            subdata['rssi_'+mac] = -200
            subdata['state_'+mac] = 0
