...
sensors = ["000000000101", "000000000201", "000000000301", "000000000401"] #Listado de sensores a utilizar, vacío para autorellenar en el arranque
number_of_nodes = 4
...
for windowsettings_suffix in windowsettingslist:
    ...
    for index, rssi_row in rssis.iterrows():
        ...
        # Extraemos los valores de los sensores y quitamos los que sean 100
        filtered_rssi_row = rssi_row[sensors]
        filtered_rssi_row = filtered_rssi_row[filtered_rssi_row != 100]
        
        if len(filtered_rssi_row) < min_number_of_nodes:
            print("No hay suficientes nodos")
            continue

        # Localizamos cada dongle en el listado cargado y acumulamos su posición
        positions = []
        for dongleMac, dongle_rssi in filtered_rssi_row.items():
